...
public class RoomCleanupScheduler {
  ...
  public void cleanupRooms() {
    ...
    for (Room room : rooms) {
      ...
      if (room.getStatus() == Room.Status.WAITING && playerCount <= 1 && age.compareTo(MAX_WAITING_AGE) > 0) {
        roomRepository.delete(room);
        roomStateService.removeRoom(roomId);
        deletedDbRooms++;
        deletedMemoryRooms++;
      ...
      if (room.getStatus() == Room.Status.PLAYING && age.compareTo(MAX_PLAYING_AGE) > 0) {
        roomRepository.delete(room);
        roomStateService.removeRoom(roomId);
        deletedDbRooms++;
        deletedMemoryRooms++;
    ...
    if (removedOrphanStates > 0 || deletedDbRooms > 0 || deletedMemoryRooms > 0) {
      log.info(
        "[RoomCleanup] summary: orphanStatesRemoved={}, dbDeleted={}, memoryDeleted={}",
        removedOrphanStates,
        deletedDbRooms,
        deletedMemoryRooms
      );
