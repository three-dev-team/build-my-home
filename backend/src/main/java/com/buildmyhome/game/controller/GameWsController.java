...
public class GameWsController {
  ...
  private final ConcurrentHashMap<Long, ConcurrentHashMap<Long, String>> lastDedupeKeyByRoom =
  ...
  private GameMessage defaultGameResponse(String type, GameState gameState) {
    ...
    if (definitionTimeout > 0 && gameState.getStatusUpdatedAt() != null) {
      long elapsedSeconds =
          java.time.Duration.between(gameState.getStatusUpdatedAt(), java.time.LocalDateTime.now())
              .toSeconds();
  ...
  public void getGameState(GameMessage message, Principal principal) {
    Long roomId = message.getRoomId();
    GameState gameState = gameStateService.getGame(roomId);
    if (gameState == null) {
      if (principal == null || principal.getName() == null) return;
      simpMessagingTemplate.convertAndSendToUser(
          principal.getName(), "/queue/game", Map.of("type", "GAME_NOT_FOUND"));
      return;
    }
    ...
    if (memberId != null) {
      ...
      if (player != null && player.isDisconnected()) {
        gameStateService.rejoinPlayer(roomId, memberId);
      }
    }
  ...
  public void startGame(GameMessage message) {
    ...
    for (RoomPlayerState player : room.getPlayers().values()) {
      GamePlayerState gps =
          new GamePlayerState(player.getMemberId(), player.getNickname(), player.getCharacterId());
      gameState.addPlayer(gps);
    }

    //      gameState.setTotalRounds(3); // TODO: 테스트용! 원래: room.getTotalRounds()
    //    for (RoomPlayerState player : room.getPlayers().values()) {
    //      // Todo: 테스트용입니다!!! 지울 것!!!!
    //      GamePlayerState gps =
    //          new GamePlayerState(player.getMemberId(), player.getNickname(),
  ...
  public void rollForOrder(GameMessage message, Principal principal) {
    ...
    synchronized (gameState) {
      ...
      // 모든 인원이 다 뽑았는지 체크
      boolean allDone =
          gameState.getPlayers().values().stream().allMatch((p) -> p.getOrderDiceValue() != null);

      if (allDone) {
        // 높은 숫자 순으로 정렬해서 turnOrder 생성
        List<Long> sortedTurnOrder =
        ...
        gameState.setPlayersYetToPlay(new HashSet<>(sortedTurnOrder)); // ★ 라운드 1 초기화
      }

      GameMessage response =
          defaultGameResponse(allDone ? "ALL_DICE_ROLLED" : "DICE_ROLLED", gameState);
      simpMessagingTemplate.convertAndSend("/topic/games/" + roomId, response);
    }
  }
  ...
  public void selectDice(GameMessage message, Principal principal) {
    ...
    synchronized (gameState) {
      ...
      simpMessagingTemplate.convertAndSend(
          "/topic/games/" + roomId, defaultGameResponse("DICE_SELECTED", gameState));
    }
  }
          ...
          case "FISHING_INTRO_NEXT":
            {
              ...
              if (!memberId.equals(gameState.getCurrentPlayerId())) break;
              boolean baitAvailable =
                  player.getShopItems() != null
                      && player.getShopItems().stream()
                          .anyMatch(it -> it == ShopItemType.FISHING_CHANCE);
