...
public class FishingSession {
    ...
    // MEDIUM에서 2연속 타격 단계
    private int stage; // 1: 1단계 진행 중, 2: 2단계 진행 중
    ...
    private long lastHitActionAtMs; // 마지막으로 클릭한 시간(epoch ms)

    // LARGE 릴링 판정
    private double progress; // 목표까지 얼마나 진행했는지(진행률,0~100)
    private double tension;  // 장력(줄이 끊어질 위험도,0~100)
    private boolean isReeling; // 현재 사용자가 릴을 “감는 중인지” 상태
    private long lastStateUpdateAtMs; // 마지막으로 progress/tension을 업데이트한 실제 시각(epoch ms)

    private double progressPerMs; // 1ms당 progress가 얼마나 증가하는지
    private double tensionUpPerMs; // 릴을 감는 중(`isReeling=true`)일 때, 1ms당 tension이 얼마나 올라가는지
    private double tensionDownPerMs; // 릴을 멈췄을 때(`isReeling=false`) 또는 안전 상태에서, 1ms당 tension이 얼마나 내려가는지
}
