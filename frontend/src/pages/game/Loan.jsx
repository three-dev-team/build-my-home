...
const Loan = ({
    ...
    const handleKeypadPress = (num) => {
        ...
        if (Number(nextValStr) > MAX_LOAN_AMOUNT) {
            // 입력하려는 값이 최대값보다 크면 최대값으로 고정 (선택 사항)
            // 또는 입력을 막을 수도 있음. 여기서는 최대값으로 고정.
    ...
    const handleConfirm = (type) => {
        console.log(
            `>>> handleConfirm [${type}] Triggered. isMyTurn: ${isMyTurn}, isBankTile: ${isBankTile}`,
        );

        // 은행/ATM 불문하고 내 턴이 아니면 동작 중단
        if (!isMyTurn) {
            console.warn("내 턴이 아니므로 동작 중단");
            return;
        }

        if (!onAction) {
            console.error("onAction 함수가 전달되지 않음!");
            return;
        }
        ...
        console.log(
            `>>> Requested Amount: ${finalAmount}, MIN_LIMIT: ${MIN_LOAN_AMOUNT}`,
        );
        ...
        >
            <div className="absolute top-8 w-full flex flex-col items-center gap-3">
                ...
                {(mode === "menu" || mode === "apply" || mode.startsWith("atm_")) && (
                    <div
                        className="flex items-center gap-4 bg-white px-8 py-2 rounded-full border-4 border-[#82ccdd] shadow-xl">
            ...
            >
                {/* 유저 라벨 */}
                <div
                    className="absolute -top-6 left-12 bg-[#5eb347] px-8 py-2 rounded-2xl text-white text-2xl font-bold shadow-lg z-10">
                    {isBankTile ? "🏦 은행 창구" : "🏧 ATM 기기"} - {currentPlayerName} 님
                </div>

                <div className="absolute top-6 right-12 flex flex-col items-end">
                    <div
                        className="bg-[#b19149] px-6 py-2 rounded-full border-4 border-white text-white text-2xl shadow-md">
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        {mode === "apply" && isBankTile && (
                            ...
                            >
                                ...
                                <div
                                    className="flex items-center gap-12 bg-[#f9f9f9] p-12 rounded-[50px] border-4 border-[#7ed321] shadow-inner">
                                    <motion.button
                                        whileTap={{scale: 0.8}}
                            ...
                            !isBankTile && (
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-center mb-8 w-full">
                                        ...
                                        <div
                                            className="bg-[#f9f9f9] w-3/4 py-4 px-8 rounded-[30px] border-4 border-[#7ed321] flex justify-end items-center shadow-inner mb-2">
            ...
            <div className="absolute bottom-5 left-10 flex flex-col items-center">
                ...
                >
                    ...
                    {/* 말풍선 꼬리 (중앙 아래로) */}
                    <div
                        className="absolute bottom-[-18px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[18px] border-l-transparent border-r-[18px] border-r-transparent border-t-[18px] border-t-[#5eb347]"/>
                </motion.div>
