  ...
  const handleConfirm = (type) => {
    ...
    if (type === 'REPAY') {
      ...
      if (finalAmount > userLoan) {
        alert(`빚(${userLoan.toLocaleString()}벨)보다 더 많이 갚을 필요는 없다구리!`);
        return;
      }
    } else if (type === 'LOAN') {
      ...
      >
        ...
        >
          ...
          <div className="absolute -top-6 left-1/2 -translate-x-1/2 flex items-center gap-4 bg-white px-8 py-2 rounded-full border-4 border-[#82ccdd] shadow-xl">
            <span className="text-xl text-gray-400 font-bold uppercase tracking-widest">Time Left</span>
          ...
          {/* 메시지 */}
          <h2 className="text-6xl font-black text-[#5eb347] mb-4">{isBankTile ? '너굴 은행' : '너굴 ATM'}</h2>
        ...
        >
          ...
          <img src="/images/tom-nook.png" alt="너굴" className="w-80 h-80 object-contain z-30" />
        </motion.div>
      </motion.div>
    );
  }
  ...
  return (
    ...
    >
      <div className="absolute top-8 w-full flex flex-col items-center gap-3">
        {/* 타이머 */}
        {(mode === 'menu' || mode === 'apply' || mode === 'repay' || mode.startsWith('atm_')) && (
      ...
      >
        ...
        <div className="absolute top-6 right-12 flex flex-col items-end">
          ...
          {userLoan > 0 && (
            <span className="text-red-500 font-bold mt-1 text-lg">상환액: {userLoan.toLocaleString()}벨</span>
          )}
        </div>

        <div className="text-center mt-6 mb-10">
          <h1 className="text-7xl font-bold text-[#5eb347]">{isBankTile ? '너굴 은행' : '너굴 ATM'}</h1>
        </div>
        ...
        <div className="w-full flex flex-col items-center min-h-[320px] justify-center">
          <AnimatePresence mode="wait">
            ...
            {mode === 'apply' && isBankTile && (
              ...
              >
                <div className="text-3xl text-gray-500">얼마나 필요하냐구리?</div>
                <div className="flex items-center gap-12 bg-[#f9f9f9] p-12 rounded-[50px] border-4 border-[#7ed321] shadow-inner">
                  ...
                  <div className="flex flex-col items-center min-w-[240px]">
                    <div className="flex items-baseline gap-2">
                      <span className="text-9xl font-black text-[#5a4a42]">{amount}</span>
            ...
            {mode === 'repay' && isBankTile && (
              <motion.div
                ...
                className="flex flex-col items-center gap-8"
              >
                <div className="text-3xl text-gray-500">얼마나 갚을거냐구리?</div>
                <div className="flex items-center gap-12 bg-[#f9f9f9] p-12 rounded-[50px] border-4 border-[#5eb347] shadow-inner">
                  ...
                  <motion.button
                    whileTap={{ scale: 0.8 }}
                    onClick={() => setAmount(Math.min(Math.min(userLoan, userBell), amount + 50))}
                    className="text-8xl text-[#5eb347] font-black"
                  >
                    ＋
                  </motion.button>
            ...
            {/* ATM 전용 키패드 화면 */}
            {(mode === 'atm_input_loan' || mode === 'atm_input_repay') && !isBankTile && (
              <motion.div
                key="atm-input"
              ...
              >
                <div className="flex flex-col items-center mb-8 w-full">
                  <h3 className="text-3xl text-[#5a4a42] font-bold mb-4">
                    {mode === 'atm_input_loan' ? '빌릴 금액을 입력하라구리' : '갚을 금액을 입력하라구리'}
                  </h3>
                  <div className="bg-[#f9f9f9] w-3/4 py-4 px-8 rounded-[30px] border-4 border-[#7ed321] flex justify-end items-center shadow-inner mb-2">
                    ...
                    <span className="text-3xl text-[#5a4a42] ml-3 font-bold">벨</span>
                  </div>

                  <div className="h-8">
                    {mode === 'atm_input_loan' && Number(keypadAmount) > 0 && (
                      <p className="text-red-500 font-bold text-xl animate-bounce">
                        ⚠️ 수수료 {Math.floor(Number(keypadAmount) * 0.1).toLocaleString()}벨 발생! (총 빚:{' '}
                        {(Number(keypadAmount) * 1.1).toLocaleString()}벨)
                      </p>
                    )}
                    {mode === 'atm_input_repay' && (
                      <p className="text-[#82ccdd] font-bold text-xl">현재 빚: {userLoan.toLocaleString()}벨</p>
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-6 w-full px-8 h-64">
                  ...
                  <div className="flex flex-col gap-4 w-1/4">
                    <button
                      onClick={() => handleConfirm(mode === 'atm_input_loan' ? 'LOAN' : 'REPAY')}
            ...
            {(mode === 'processing' || mode === 'success') && (
              <motion.div initial={{ scale: 0.5 }} animate={{ scale: 1 }} className="flex flex-col items-center gap-6">
                ...
                <p className="text-5xl font-black text-[#5a4a42] text-center leading-tight">
                  {mode === 'processing' ? '서버에 서류 보내는 중...' : feedbackMsg}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
      ...
      >
        <div className="mb-2 bg-white p-6 rounded-[40px] border-4 border-[#5eb347] shadow-2xl relative max-w-sm z-40">
          <p className="text-2xl font-bold text-[#5a4a42] leading-relaxed text-center">
            {isBankTile ? '은행 창구는 수수료 무료!\n맘껏 빌려라구리!' : 'ATM은 편리한 만큼\n수수료 10%가 든다구리!'}
          </p>
          <div className="absolute bottom-[-18px] left-1/2 -translate-x-1/2 w-0 h-0 border-l-[18px] border-l-transparent border-r-[18px] border-r-transparent border-t-[18px] border-t-[#5eb347]" />
        </div>
        <img src="/images/tom-nook.png" alt="너굴" className="w-80 h-80 object-contain z-30" />
...
const BankButton = ({ onClick, color, icon, label, subLabel, disabled }) => (
  <motion.button
    ...
    className={`w-72 h-72 rounded-[60px] flex flex-col items-center justify-center text-white shadow-[0_15px_0_rgba(0,0,0,0.1)] transition-all
            ${disabled ? 'bg-gray-300 grayscale opacity-40 cursor-not-allowed' : 'cursor-pointer'}`}
  ...
  >
    ...
    <span className="text-lg mt-2 opacity-80 bg-black/20 px-4 py-1 rounded-full">{subLabel}</span>
  </motion.button>
);
