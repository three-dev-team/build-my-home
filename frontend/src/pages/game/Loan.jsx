...
/**
 * @param {number} userBalance - 현재 유저 잔액
 * @param {number} currentLoan - 현재 유저 대출금
 * @param {boolean} isActivePlayer - 현재 조작 권한이 있는 유저인지
 * @param {string} activePlayerName - 현재 은행을 이용 중인 유저의 이름
 * @param {boolean} isBankSquare - 현재 위치가 '대출 칸'인지 여부 (false면 수수료 10% 추가)
...
const Loan = ({
  ...
  // 서버에서 20초 후 자동 종료되므로, 시각적으로만 보여주는 20초 카운트다운
  ...
  const handleConfirm = (type) => {
    ...
    setTimeout(() => {
      if (type === "LOAN") {
        const feeText = !isBankSquare ? " (수수료 10% 포함)" : "";
        onAction("LOAN", amount, isBankSquare);
        setFeedbackMsg(`${amount}벨 대출 완료!${feeText} 빚도 실력이다구리!`);
      } else {
        onAction("REPAY", currentLoan);
        ...
        <div className="flex items-center gap-4 bg-white px-8 py-2 rounded-full border-4 border-[#82ccdd] shadow-xl">
          ...
          <span
            className={`text-4xl font-black ${
              timeLeft <= 10 ? "text-red-500 animate-pulse" : "text-[#34495e]"
            }`}
          >
            {timeLeft}s
          </span>
        </div>
        {!isBankSquare && isActivePlayer && (
          <motion.div
            initial={{ y: -20 }}
            animate={{ y: 0 }}
      ...
      >
        ...
        <div className="absolute top-6 right-12 flex flex-col items-end">
          <div className="bg-[#b19149] px-6 py-2 rounded-full border-4 border-white text-white text-2xl shadow-md">
            {userBalance.toLocaleString()} <small>벨</small>
          </div>
          {currentLoan > 0 && (
            <span className="text-red-500 font-bold mt-1 text-lg">
              상환액: {currentLoan.toLocaleString()}벨
            </span>
          )}
        </div>
        ...
        <div className="text-center mt-6 mb-10">
          ...
          {!isActivePlayer && (
            <p className="text-2xl text-[#82ccdd] mt-4 font-bold">
              {activePlayerName} 님이 신중하게 고민 중입니다...
            </p>
          )}
        </div>
              ...
              >
                <div className="grid grid-cols-2 gap-12">
                  <BankButton
                    ...
                    subLabel={!isBankSquare ? "수수료 10%" : "수수료 없음"}
                  />
                  <BankButton
                    ...
                    icon="💵"
                    label="빚 갚기"
                    disabled={!isActivePlayer || currentLoan <= 0}
                    subLabel="즉시 상환"
                  />
                </div>
                <button
