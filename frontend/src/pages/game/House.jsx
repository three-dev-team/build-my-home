...
const House = ({ player, isMyTurn, onClose, onAction, radishPrice = 0 }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 bg-[#ffffff] z-[100]">
      {/* 상시 닫기 버튼 */}
      {isMyTurn && (
        <button
          onClick={onClose}
        ...
        <div className="w-full h-full flex flex-col items-center justify-center p-8">
          ...
          <div className="flex gap-6 overflow-x-auto pb-4 px-4">
            {HOUSE_LEVEL_MAP.filter((level) => level.level > 0).map((level) => {
              return (
                <div key={level.key} className={'flex-shrink-0 w-48 rounded-xl p-4 text-center'}>
                  {/* TODO: 이미지로 대체 */}
                  <div className="w-28 h-28 mx-auto bg-gray-300 rounded-lg mb-2 flex items-center justify-center text-4xl">
                    🏠
                  </div>
      ...
      {step === 2 && (
        <div className="w-full h-full flex flex-col items-center justify-center p-8">
          {!nextHouseLevel ? (
            <>
              <p className="text-2xl mb-8 text-black text-center">이미 최고 등급의 집이라구리! 🎉</p>
              <button
                onClick={() => setStep(0)}
                disabled={!isMyTurn}
                className="mt-6 px-8 py-4 bg-gray-300 hover:bg-gray-400 rounded-full font-bold text-lg"
            ...
            <>
              <p className="text-2xl mb-8 text-black text-center">
                ...
                재료를 다 구해왔구나!
                <br />
                이제 <span className="font-bold text-yellow-300">{nextLevelData?.name}</span>를 건설할 수 있다구리.
                <br />
                진행하겠냐구리?
              </p>
      ...
      {step === 11 && (
        <div className="w-full h-full flex flex-col items-center justify-center p-8">
          ...
          <div className="bg-white/90 rounded-xl p-6 shadow-md w-full max-w-[420px]">
            ...
            {!isMyTurn && (
              <p className="text-sm text-gray-600 mt-4 text-center">지금은 내 차례가 아니라 판매할 수 없어.</p>
            )}
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="w-full h-full flex flex-col items-center justify-center p-8">
          ...
          <p className="text-2xl mb-8 text-black">{HOUSE_DETAILS[houseLevel]?.name} 업그레이드 완료!</p>
          <button
            onClick={onClose}
            disabled={!isMyTurn}
            className="px-8 py-4 bg-yellow-400 hover:bg-yellow-500 rounded-full font-bold text-lg"
