            ...
            {modal.isOpen && (
                <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="relative w-[380px] bg-[#fdf6e3] rounded-[40px] border-[6px] border-[#8b5a2b] shadow-2xl p-8 flex flex-col items-center animate-in zoom-in-95">
                        ...
                        <button onClick={closeModal} className="mt-6 px-10 py-2 bg-[#8b5a2b] text-white font-black rounded-full active:scale-95 transition-all">확 인</button>
                    </div>
                </div>
            )}
                ...
                <form onSubmit={handleJoin} className="mt-10 w-full space-y-4">
                    ...
                    {isEmailSent && !isEmailVerified && (
                        <div className="flex gap-2 animate-in slide-in-from-top-2">
                            <div className="relative flex-1">
                                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xl">🔑</span>
                                <input type="text" placeholder="인증번호 6자리" value={authCode} onChange={(e) => setAuthCode(e.target.value)} className="w-full bg-[#efe7d1] rounded-2xl py-3 pl-12 pr-16 text-[#5d4037] font-bold outline-none" maxLength={6} />
                                <span className={`absolute right-4 top-1/2 -translate-y-1/2 font-bold text-sm ${timeLeft < 60 ? "text-red-500 animate-pulse" : "text-[#8b5a2b]"}`}>
                                    {formatTime(timeLeft)}
                                </span>
                            </div>
                            <button type="button" onClick={handleVerifyCode} className="bg-[#8b5a2b] text-white px-4 rounded-xl font-bold text-xs">확인</button>
                        </div>
                    )}
                    ...
                    <div className="relative">
                        ...
                        />
                        {isPasswordInvalid && <p className="text-red-500 text-xs font-bold mt-1 ml-4 animate-in fade-in">❌ 영문, 숫자, 특수문자 포함 8~16자로 입력해주세요!</p>}
                    </div>
                    ...
                    <div className="relative">
                        ...
                        />
                        {isPasswordMismatch && <p className="text-red-500 text-xs font-bold mt-1 ml-4 animate-in fade-in">❌ 비밀번호가 일치하지 않습니다!</p>}
                    </div>
                    ...
                    <div className="flex gap-2">
                        <div className="relative flex-1">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-xl">🍃</span>
                            <input type="text" placeholder="닉네임 입력" value={nickname} onChange={(e) => { setNickname(e.target.value); setIsNicknameChecked(false); }} className={`w-full bg-[#efe7d1] rounded-2xl py-3 pl-12 pr-4 text-[#5d4037] font-bold outline-none transition-all ${isNicknameChecked ? "ring-2 ring-green-500" : "focus:ring-4 ring-[#8b5a2b]/20"}`} />
                        </div>
                        <button type="button" onClick={handleCheckNickname} className="bg-[#bc8a5f] text-white px-4 rounded-xl font-bold text-xs active:scale-95 transition-all">중복체크</button>
                    </div>

                    <button type="submit" className="w-full bg-[#8b5a2b] hover:bg-[#6d4622] text-[#fdf6e3] py-4 rounded-[25px] text-xl font-black shadow-lg transition-all active:scale-95">주민 가입 완료!</button>
                </form>
